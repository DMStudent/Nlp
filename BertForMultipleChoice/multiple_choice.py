...
def file_based_convert_examples_to_features(
  ...
  for example_index, example in enumerate(examples):
      if example_index % 100000 == 0:
          tf.logging.info("convert example %d of %d" % (example_index, len(examples)))
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  with tf.variable_scope("final_layer"):
    ...
    reshaped_logits = tf.reshape(logits, [-1, num_labels], name="logits")
    labels = tf.reshape(labels, [-1, num_labels])
    mean_labels = tf.reduce_mean(labels, axis=1)
    probabilities = tf.math.sigmoid(reshaped_logits, name="probabilities")
    loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=mean_labels, logits=reshaped_logits)
    ...
    return (loss, mean_labels, reshaped_logits, probabilities)
...
def serving_input_fn():
    ...
    # is_real_example = tf.placeholder(dtype=tf.bool, shape=[None], name="is_real_example")
...
def serving_input_receiver_fn():
  ...
  feature_columns = [
      tf.feature_column.numeric_column(dtype=tf.int64, key='input_ids', shape=[FLAGS.max_seq_length]),
      tf.feature_column.numeric_column(dtype=tf.int64, key='input_mask', shape=[FLAGS.max_seq_length]),
      tf.feature_column.numeric_column(dtype=tf.int64, key='segment_ids', shape=[FLAGS.max_seq_length]),
      tf.feature_column.numeric_column(dtype=tf.int64, key='label_ids', shape=[1])
  ]
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    (total_loss, labels, logits, probabilities) = create_model(
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      logging_hook = tf.train.LoggingTensorHook({
          "loss": total_loss, "probabilities":probabilities[:5],
          "labels":labels[:5]}, every_n_iter=50)
  ...
  if FLAGS.do_predict:
    ...
    with tf.gfile.GFile(output_predict_file, "w") as writer:
      ...
      for (i, prediction) in enumerate(result):
        ...
        output_line = label_list[max_idx] + "\t" + "\t".join(
